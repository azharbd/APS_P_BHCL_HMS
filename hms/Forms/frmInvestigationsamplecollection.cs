    ...
    public partial class frmInvestigationsamplecollection : Form
    {
        ...
        private void saveData()
        {
                ...
                for (int i = 0; i < dgvSampleCollection.Rows.Count - 1; i++)
                {
                    if ((bool)dgvSampleCollection.Rows[i].Cells[0].FormattedValue == true)
                    {
                        if (strIDs.ToString() == "")
                        {
                            ...
                            strSqlInsertHistory = " (" + dgvSampleCollection.Rows[i].Cells[9].Value.ToString() + ", 1)";
                        }
                        else
                        {
                            strIDs = strIDs + ", " + dgvSampleCollection.Rows[i].Cells[9].Value.ToString();
                            strSqlInsertHistory = strSqlInsertHistory + ", (" + dgvSampleCollection.Rows[i].Cells[9].Value.ToString() + ", 1)";
                        }
                    }
                }
            }
            ...
            if (strIDs.ToString() != "")
            {
                ...
                strSqlInsertHistory = "insert into Investigation_Serviceitem_History (IST_ID, Status) values " + strSqlInsertHistory;
